...
def getURL():
    ...
    for col_index, col_inner in enumerate(soup.select('.col_inner')):
        ...
        for thumb_index, thumb_tag in enumerate(col_inner.select('.thumb')):
            ...
            # 웹툰 제목 추출
            comic_title = str(soup.select('.comicinfo h2')[0].contents[0]).strip()
            # 에피소드 제목 추출
            ...
            for content in soup.select('td a'):
                if content['href'].find('/webtoon') != -1 and len(content.contents) != 1:
                    recently_url = 'https://comic.naver.com' + content['href']
                    inside_thumb_url = content.contents[1]['src']
                    semi_title = content.contents[1]['alt']
                    insertdb(comic_title, semi_title, inside_thumb_url)
...
def crawl_naver_webtoon(episode_url):
    ...
    for img_tag in soup.select('.wt_viewer img'):
        ...
        # db에 저장
        insertdb(comic_title, ep_title, image_file_data)

    print('Completed !')

def insertdb(title, semi_title, image_data):
    try: 
        sql_insert_blob_query = """ INSERT INTO naver_serial_webtoon_inside
                            (title, semi_title, image_data) VALUES (%s,%s,%s)"""

        # Convert data into tuple format
        insert_blob_tuple = (title, semi_title, image_data)
